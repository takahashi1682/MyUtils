    ...
    public class GradientImage : BaseMeshEffect
    {
        ...
        protected void SetVertexColor()
        {
            ...
            for (var i = 0; i < _vertexList.Count; i++)
            {
                ...
                vertex.color =
                    _gradient.Evaluate(Mathf.InverseLerp(_minMax[0], _minMax[1], vertex.position[_axisIndex]));
        ...
        private void Divide(int startIndex, float targetValue)
        {
            ...
            var minMaxUV = new Vector2(_vertexList[startIndex].uv0[_axisIndex],
                _vertexList[startIndex].uv0[_axisIndex]);
            var minMaxPos = new Vector2(_vertexList[startIndex].position[_axisIndex],
                _vertexList[startIndex].position[_axisIndex]);
            for (var i = 1; i < 6; i++)
            {
            ...
            foreach (var index in indexes)
            {
                ...
                vertex.position = pos;
                var uv = vertex.uv0;
                uv[_axisIndex] = Mathf.Lerp(minMaxUV[0], minMaxUV[1],
                    Mathf.InverseLerp(minMaxPos[0], minMaxPos[1], pos[_axisIndex]));
                vertex.uv0 = uv;
